...
# Generate an Ed25519 signing keypair for piwrite release signing.
#
...
OUT_DIR="${1:-$HOME/.piwrite/signing}"

usage() {
    cat <<EOF
Usage: $0 [OUT_DIR]

Generate an Ed25519 keypair for signing piwrite releases.

  OUT_DIR  Directory to write the keys (default: \$HOME/.piwrite/signing)

Writes:
  OUT_DIR/secret.pem   Secret key — KEEP OFFLINE, never commit
  OUT_DIR/public.pem   Public key — commit and pin in the installers

After generating, copy the contents of public.pem into the committed
piwrite-signing-key.pub file (and netinstall.sh's pinned key).
EOF
}
